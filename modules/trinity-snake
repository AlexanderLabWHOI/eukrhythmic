...
rule trinity:
    ...
    shell:
        '''
        echo {params.left}
        Trinity --seqType fq --max_memory {params.maxmem}G --CPU {params.CPUs} --left {params.left} --right {params.right} --output {params.outdir} --NO_SEQTK 2> {log.err} 1> {log.out}
        '''
...
rule trinity_SE:
    ...
    shell:
        '''
        Trinity --seqType fq --max_memory {params.maxmem}G --CPU {params.CPUs} --bflyCalculateCPU --single {params.single} --output {params.outdir} --NO_SEQTK 2> {log.err} 1> {log.out}
        '''
