...
rule trimmomatic:
    input:
        r1 = lambda filename: expand(os.path.join(INPUTDIR, "{sampnames}"), sampnames = get_filenames_trim(filename.sample, "left")),
        #os.path.join(INPUTDIR, "{sample}_R1" + SUFFIX),
        r2 = lambda filename: expand(os.path.join(INPUTDIR, "{sampnames}"), sampnames = get_filenames_trim(filename.sample, "right")),
        #os.path.join(INPUTDIR, "{sample}_R2" + SUFFIX),
        adapters = ADAPTER
    output:
    ...
    log:
        err = os.path.join("logs","trimmomatic","PE_{sample}_err.log"),
        out = os.path.join("logs","trimmomatic","PE_{sample}_out.log")
    conda: "../envs/trimmomatic-env.yaml"
    shell:
        '''
...
rule trimmomatic_SE:
    input:
        r1 = lambda filename: expand(os.path.join(INPUTDIR, "{sampnames}"), sampnames = get_filenames(filename.sample, "left")),
        adapters = ADAPTER
    output:
        p1 = os.path.join(OUTPUTDIR, "firsttrim", "{sample}_1.trimmed.fastq.gz"),
        s1 = os.path.join(OUTPUTDIR, "firsttrim", "{sample}_1.unpaired.fastq.gz")
    log:
        err = os.path.join("logs","trimmomatic","SE_{sample}_err.log"),
        out = os.path.join("logs","trimmomatic","SE_{sample}_out.log")
    conda: "../envs/trimmomatic-env.yaml"
    shell:
        '''
